...
-->
	...
	<xsl:template match="*[local-name()='load' and not(@interface)]" mode="body">
		<xsl:text>		var </xsl:text><xsl:value-of select="@var"/><xsl:text>Req = new XMLHttpRequest();
</xsl:text>
		<xsl:text>		</xsl:text>
		<xsl:value-of select="@var"/>
		<xsl:text>Req.open("GET", "files/</xsl:text>
		...
		<xsl:text>		</xsl:text>
		<xsl:value-of select="@var"/>
		<xsl:text> = new DOMParser().parseFromString(</xsl:text>
		<xsl:value-of select="@var"/>
